...
public class SearchToolPanel extends FlowPanel {
	...
	private FlowPanel initConeDetails() {
		...
		CommonEventBus.getEventBus().addHandler(IsShowingCoordintesInDegreesChangeEvent.TYPE, () -> {
			try {
				String[] coordStr = updateCoordinateFormat(new RaPosition(raText.getText()),
						new DecPosition(decText.getText(), true));
		...
		CommonEventBus.getEventBus().addHandler(AladinLiteSelectSearchAreaEvent.TYPE, searchAreaEvent -> {
			if (searchAreaEvent != null && searchAreaEvent.getSearchArea() != null
					&& searchAreaEvent.getSearchArea().isCircle()) {
				detailContainer.removeStyleName(inputErrorClassName);
				...
				double[] coords = convertCoordinatesToFrame(Double.toString(coordObj.getRaDeg()),
						Double.toString(coordObj.getDecDeg()), CoordinatesFrame.J2000);
	...
	public boolean createConicalSearchArea(String ra, String dec, String radius) {
		...
		try {
			CoordinatesFrame cooFrame = CoordinatesFrame
					.valueOf(AladinLiteWrapper.getAladinLite().getCooFrame().toUpperCase());
			double[] coords = convertCoordinatesToFrame(ra, dec, cooFrame, CoordinatesFrame.J2000);

			AladinLiteWrapper.getAladinLite()
					.createSearchArea("CIRCLE ICRS " + coords[0] + " " + coords[1] + " " + radius);
			AladinLiteWrapper.getAladinLite().endSelectionMode();
		} catch (Exception ex) {
			Log.debug(ex.getMessage(), ex);
	...
	private double[] convertCoordinatesToFrame(String raStr, String decStr, CoordinatesFrame oldCooFrame,
		...
		return CoordinatesParser.convertCoordsToDegrees(new ClientRegexClass(), raStr + " " + decStr, oldCooFrame,
				newCooFrame);
	}
