...
public class TAPSSOService extends AbstractTAPService {
    ...
    public String getCount(String ssoName, ESASkySSOObjType ssoType) {
        ...
        String adql = "select b.* from sso_pipeline.ssoid as a join sso_pipeline.count_table as b on a.sso_oid = b.sso_oid where a.sso_name = '"
                + ssoName + "' and a.sso_type = '" + dbSSOType + "'";

        if (ssoType == ESASkySSOObjType.ASTEROID) {
            dbSSOType = "aster";
            adql = "select b.* from sso_pipeline.ssoid as a join sso_pipeline.count_table as b on a.sso_oid = b.sso_oid where a.sso_name = '"
                    + ssoName + "' and a.sso_type = '" + dbSSOType + "'";
        } else if (ssoType == ESASkySSOObjType.COMET) {
            dbSSOType = "comet";
            adql = "select b.* from sso_pipeline.ssoid as a join sso_pipeline.count_table as b on a.sso_oid = b.sso_oid where a.sso_id = '"
                    + ssoName + "' and a.sso_type = '" + dbSSOType + "'";
