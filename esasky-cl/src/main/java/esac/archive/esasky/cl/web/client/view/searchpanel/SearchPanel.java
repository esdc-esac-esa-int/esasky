...
public class SearchPanel extends Composite implements SearchPresenter.View {
    ...
    private void initView() {
        ...
        searchTextBox.setVisible(Modules.getModule(EsaSkyWebConstants.MODULE_SEARCH_BOX));
        searchTextBox.addValueChangeHandler(event -> {
            ensureClearTextButtonVisibilty();
            GUISessionStatus.setIsTrackingSSO(false);
        });
        searchTextBox.addKeyPressHandler(event -> {
            ensureClearTextButtonVisibilty();
            GUISessionStatus.setIsTrackingSSO(false);
        });
        searchTextBox.addKeyUpHandler(event -> ensureClearTextButtonVisibilty());
        ...
        searchWidthAnimation.addObserver(currentPosition -> {
    ...
    private Widget createSearchToolboxBtn() {
        ...
        CommonEventBus.getEventBus().addHandler(IsInScienceModeChangeEvent.TYPE, this::setSearchToolVisibility);
    ...
    private void setSearchToolVisibility() {
        if (!GUISessionStatus.getIsInScienceMode() || !Modules.getModule(EsaSkyWebConstants.MODULE_SEARCH_TOOL)) {
