...
;(function (global, factory) {
			...
			this.table.element.addEventListener("copy", function (e) {
				...
				if (!_this45.blocked) {
					...
					if (_this45.customSelection) {
						...
						var list = _this45.table.modules.export.generateExportList(_this45.table.options.clipboardCopyConfig, _this45.table.options.clipboardCopyStyled, _this45.rowRange, "clipboard");
		...
		if (config.children) {
			...
			config.children.forEach(function (childRow) {
				if (childRow instanceof Row) {
					...
					if (recurse) {
						output = output.concat(_this51.getTreeChildren(childRow, component, recurse));
					}
				}
			});
		}
	...
	Export.prototype.rowLookup = function (range) {
		...
		if (typeof range == "function") {
			range.call(this.table).forEach(function (row) {
				row = _this54.table.rowManager.findRow(row);
	...
	Export.prototype.generateColumnGroupHeaders = function () {
		...
		columns.forEach(function (column) {
			var colData = _this55.processColumnGroup(column);
	...
	Export.prototype.processColumnGroup = function (column) {
		...
		if (subGroups.length) {
			...
			subGroups.forEach(function (subGroup) {
				var subGroupData = _this56.processColumnGroup(subGroup);
	...
	Export.prototype.bodyToExportRows = function (rows) {
		...
		this.table.columnManager.columnsByIndex.forEach(function (column) {
			if (_this57.columnVisCheck(column)) {
		...
		rows = rows.filter(function (row) {
			switch (row.type) {
				case "group":
					return _this57.config.rowGroups !== false;
					break;

				case "calc":
					return _this57.config.columnCalcs !== false;
					break;

				case "row":
					return !(_this57.table.options.dataTree && _this57.config.dataTree === false && row.modules.dataTree.parent);
					break;
			}
		...
		rows.forEach(function (row, i) {
			var rowData = row.getData(_this57.colVisProp);
			var exportCols = [];
			var indent = 0;
			...
			switch (row.type) {
				...
				case "row":
					...
					if (_this57.table.options.dataTree && _this57.config.dataTree !== false) {
	...
	Export.prototype.genereateTableElement = function (list) {
		...
		list.forEach(function (row, i) {
			switch (row.type) {
				case "header":
					headerEl.appendChild(_this58.genereateHeaderElement(row, setup, styles));
					break;

				case "group":
					bodyEl.appendChild(_this58.genereateGroupElement(row, setup, styles));
					break;

				case "calc":
					bodyEl.appendChild(_this58.genereateCalcElement(row, setup, styles));
					break;

				case "row":
					var rowEl = _this58.genereateRowElement(row, setup, styles);
					_this58.mapElementStyles(i % 2 && styles.evenRow ? styles.evenRow : styles.oddRow, rowEl, ["border-top", "border-left", "border-right", "border-bottom", "color", "font-weight", "font-family", "font-size", "background-color"]);
	...
	Export.prototype.genereateHeaderElement = function (row, setup, styles) {
		...
		row.columns.forEach(function (column) {
			if (column) {
				...
				_this59.mapElementStyles(column.component.getElement(), cellEl, ["text-align", "border-top", "border-left", "border-right", "border-bottom", "background-color", "color", "font-weight", "font-family", "font-size"]);
				_this59.mapElementStyles(column.component._column.contentElement, cellEl, ["padding-top", "padding-left", "padding-right", "padding-bottom"]);

				if (column.component._column.visible) {
					_this59.mapElementStyles(column.component.getElement(), cellEl, ["width"]);
				} else {
				...
				if (column.component._column.parent) {
					_this59.mapElementStyles(column.component._column.parent.groupElement, cellEl, ["border-top"]);
				}
	...
	Export.prototype.genereateRowElement = function (row, setup, styles) {
		...
		row.columns.forEach(function (col) {
			...
			if (col) {
				...
				if (_this60.table.modExists("format") && _this60.config.formatCells !== false) {
					value = _this60.table.modules.format.formatExportValue(cellWrapper, _this60.colVisProp);
				} else {
					switch (typeof value === 'undefined' ? 'undefined' : _typeof(value)) {
						case "object":
				...
				if (styles.firstCell) {
					_this60.mapElementStyles(styles.firstCell, cellEl, ["padding-top", "padding-left", "padding-right", "padding-bottom", "border-top", "border-left", "border-right", "border-bottom", "color", "font-weight", "font-family", "font-size"]);
				...
				if (_this60.table.options.dataTree && _this60.config.dataTree !== false) {
				...
				if (setup.rowFormatter && _this60.config.formatCells !== false) {
		...
		if (field) {
			...
			if (editor) {
				...
				editorElement.addEventListener("focus", function (e) {
					var left = _this61.table.columnManager.element.scrollLeft;

					if (left !== _this61.table.rowManager.element.scrollLeft) {
						_this61.table.rowManager.scrollHorizontal(left);
						_this61.table.columnManager.scrollHorizontal(left);
					}
				});
	...
	Filter.prototype.filtersToArray = function (filterList, ajax) {
		...
		filterList.forEach(function (filter) {
			...
			if (Array.isArray(filter)) {
				output.push(_this62.filtersToArray(filter, ajax));
			} else {
		...
		rowSelection: function rowSelection(cell, formatterParams, onRendered) {
			...
			if (this.table.modExists("selectRow", true)) {
				...
				if (typeof cell.getRow == 'function') {
					...
					checkbox.addEventListener("change", function (e) {
						if (_this63.table.modules.selectRow.selectedRows.length) {
							_this63.table.deselectRow();
						} else {
							_this63.table.selectRow(formatterParams.rowRange);
						}
					});
	...
	FrozenColumns.prototype.layoutColumnPosition = function (allCells) {
		...
		this.leftColumns.forEach(function (column, i) {
			column.modules.frozen.margin = _this65._calcSpace(_this65.leftColumns, i) + _this65.table.columnManager.scrollLeft + "px";

			if (i == _this65.leftColumns.length - 1) {
			...
			if (column.parent.isGroup) {
				var parentEl = _this65.getColGroupParentElement(column);
				if (!leftParents.includes(parentEl)) {
					_this65.layoutElement(parentEl, column);
				...
				_this65.layoutElement(column.getElement(), column);
			}

			if (allCells) {
				column.cells.forEach(function (cell) {
					_this65.layoutElement(cell.getElement(true), column);
				});
			}
		});

		this.rightColumns.forEach(function (column, i) {
			column.modules.frozen.margin = _this65.rightPadding - _this65._calcSpace(_this65.rightColumns, i + 1) + "px";

			if (i == _this65.rightColumns.length - 1) {
				column.modules.frozen.edge = true;
			} else {
				column.modules.frozen.edge = false;
			}
			...
			if (column.parent.isGroup) {
				_this65.layoutElement(_this65.getColGroupParentElement(column), column);
			} else {
				_this65.layoutElement(column.getElement(), column);
			}
	...
	Group.prototype.createValueGroups = function () {
		...
		if (this.groupManager.allowedValues && this.groupManager.allowedValues[level]) {
			this.groupManager.allowedValues[level].forEach(function (value) {
				_this67._createGroup(value, level);
			});
		}
	};
	...
	GroupRows.prototype.getChildGroups = function (group) {
		...
		group.groupList.forEach(function (child) {
			if (child.groupList.length) {
				groupComponents = groupComponents.concat(_this68.getChildGroups(child));
			} else {
	...
	Menu.prototype.initializeColumnHeader = function (column) {
		...
		if (column.definition.headerMenu) {
			...
			headerMenuEl.addEventListener("click", function (e) {
				...
				_this70.LoadMenuEvent(column, column.definition.headerMenu, e);
			});
	...
	Menu.prototype.tapHold = function (component, menu) {
		...
		element.addEventListener("touchstart", function (e) {
			...
			tapHold = setTimeout(function () {
				...
				_this71.LoadMenuEvent(component, menu, e);
			}, 1000);
		}, { passive: true });
		...
		menu.forEach(function (item) {
			...
			if (item.separator) {
				...
				if (typeof label == "function") {
					label = label.call(_this72.table, component.getComponent());
				}
				...
				if (typeof disabled == "function") {
					disabled = disabled.call(_this72.table, component.getComponent());
				}
				...
				if (disabled) {
					...
					if (item.menu && item.menu.length) {
						itemEl.addEventListener("click", function (e) {
							e.stopPropagation();
							_this72.hideOldSubMenus(menuEl);
							_this72.loadMenu(e, component, item.menu, itemEl);
						});
					} else {
						if (item.action) {
							itemEl.addEventListener("click", function (e) {
	...
	Menu.prototype.positionMenu = function (element, parentEl, touch, e) {
		...
		setTimeout(function () {
			...
			window.addEventListener("resize", _this73.blurEvent);
