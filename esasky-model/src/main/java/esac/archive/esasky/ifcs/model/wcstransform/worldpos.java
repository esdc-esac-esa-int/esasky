...
/*
 * worldpos.c -- WCS Algorithms from Classic AIPS. February 6, 1998 Copyright (C) 1994 Associated
 * Universities, Inc. Washington DC, USA. With code added by Doug Mink, Smithsonian Astrophysical
 * Observatory
 * 
 * Module: worldpos.c Purpose: Perform forward and reverse WCS computations for 8 projections
 * Subroutine: worldpos() converts from pixel location to RA,Dec Subroutine: worldpix() converts
 * from RA,Dec to pixel location
 * 
 ...
 * library; if not, write to the Free Software Foundation, Inc., 675 Massachusetts Ave, Cambridge,
 * MA 02139, USA.
 * 
 * Correspondence concerning AIPS should be addressed as follows: Internet email: aipsmail@nrao.edu
 * Postal address: AIPS Group National Radio Astronomy Observatory 520 Edgemont Road
 * Charlottesville, VA 22903-2475 USA
 * 
 * -=-=-=-=-=-=-
 * 
 * These two ANSI C functions, worldpos() and worldpix(), perform forward and reverse WCS
 * computations for 8 types of projective geometries ("-SIN", "-TAN", "-ARC", "-NCP", "-GLS",
 * "-MER", "-AIT" and "-STG"):
 * 
 * worldpos() converts from pixel location to RA,Dec worldpix() converts from RA,Dec to pixel
 * location
 * 
 * where "(RA,Dec)" are more generically (long,lat). These functions are based on the WCS
 * implementation of Classic AIPS, an implementation which has been in production use for more than
 * ten years. See the two memos by Eric Greisen
 * 
 ...
 * documents describe the differences between these algorithms and the 1993-94 WCS draft proposal
 * (see URL below). In particular, these algorithms support ordinary field rotation, but not skew
 * geometries (CD or PC matrix cases). Also, the MER and AIT algorithms work correctly only for
 * CRVALi=(0,0). Users should note that GLS projections with yref!=0 will behave differently in this
 * code than in the draft WCS proposal. The NCP projection is now obsolete (it is a special case of
 * SIN). WCS syntax and semantics for various advanced features is discussed in the draft WCS
 * proposal by Greisen and Calabretta at:
 * 
 * ftp://fits.cv.nrao.edu/fits/documents/wcs/wcs.all.ps.Z
 * 
 * -=-=-=-
 * 
 * The original version of this code was Emailed to D.Wells on Friday, 23 September by Bill Cotton
 * <bcotton@gorilla.cv.nrao.edu>, who described it as a "..more or less.. exact translation from the
 * AIPSish..". Changes were made by Don Wells <dwells@nrao.edu> during the period October 11-13,
 * 1994: 1) added GNU license and header comments 2) added testpos.c program to perform extensive
 * circularity tests 3) changed float-->double to get more than 7 significant figures 4) testpos.c
 * circularity test failed on MER and AIT. B.Cotton found that "..there were a couple of lines of
 * code [in] the wrong place as a result of merging several Fortran routines." 5) testpos.c found 0h
 * wraparound in worldpix() and worldpos(). 6) E.Greisen recommended removal of various redundant
 * if-statements, and addition of a 360d difference test to MER case of worldpos(). 7) D.Mink
 * changed input to data structure and implemented rotation matrix.
 */
...
public class worldpos {
    ...
    public static Point2D.Double getPosition(double xpix, double ypix, WCSTransform wcs) {
        ...
        case 6: /* -MER mercator */
            ...
            if (geo1 <= 0.0)
                geo1 = 1.0;
            rat = l / geo1 + ra0;
            if (Math.abs(rat - ra0) > twopi)
                return null; /* added 10/13/94 DCW/EWG */
            dt = 0.0;
            if (geo2 != 0.0)
                dt = (m + geo3) / geo2;
            dt = Math.exp(dt);
        ...
        case 7: /* -AIT Aitoff */
            ...
            dx = Math.sin(dy + dt) / Math.sqrt((1.0 + Math.cos(dy + dt)) / 2.0) - Math.sin(dy)
                    / Math.sqrt((1.0 + Math.cos(dy)) / 2.0);
            if (dx == 0.0)
                dx = 1.0;
            geo2 = dt / dx;
        ...
        if (raout - ra0 < -twopi / 2.0)
            raout = raout + twopi;
        if (raout < 0.0)
            raout += twopi; /* added by DCW 10/12/94 */
    ...
    /*
     * Oct 26 1995 Fix bug which interchanged RA and Dec twice when coorflip Oct 31 1996 Fix CD
     * matrix use in WORLDPIX Nov 4 1996 Eliminate extra code for linear projection in WORLDPIX Nov
     * 5 1996 Add coordinate flip in WORLDPIX
     * 
     * May 22 1997 Avoid angle wraparound when CTYPE is pixel Jun 4 1997 Return without angle
     * conversion from worldpos if type is PIXEL Oct 20 1997 Add chip rotation; compute rotation
     * angle trig functions Feb 6 1998 Move coordinate exchange to correct place Feb 6 1998 Drop
     * chip rotation; more CD->rotation to WCSINIT()
     */
}
