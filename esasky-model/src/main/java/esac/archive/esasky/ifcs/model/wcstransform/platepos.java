...
/*
 * File saoimage/wcslib/platepos.c February 25, 1996 By Doug Mink, Harvard-Smithsonian Center for
 * Astrophysics
 * 
 * Module: platepos.c (Plate solution WCS conversion Purpose: Compute WCS from Digital Sky Survey
 * plate fit Subroutine: platepos() converts from pixel location to RA,Dec Subroutine: platepix()
 * converts from RA,Dec to pixel location
 * 
 * These functions are based on the astrmcal.c portion of GETIMAGE by J. Doggett and the
 * documentation distributed with the Digital Sky Survey.
 */
...
public class platepos {
    ...
    public static Point2D.Double getPosition(double xpix, double ypix, WCSTransform wcs) {
        ...
        /*
         * Ignore magnitude and color terms double mag = 0.0; double color = 0.0;
         */
        ...
        xi = wcs.amd_x_coeff[0] * xmm + wcs.amd_x_coeff[1] * ymm + wcs.amd_x_coeff[2]
                + wcs.amd_x_coeff[3] * xmm2 + wcs.amd_x_coeff[4] * xmm * ymm + wcs.amd_x_coeff[5]
                * ymm2 + wcs.amd_x_coeff[6] * (x2y2) + wcs.amd_x_coeff[7] * xmm3
        ...
        /*
         * Ignore magnitude and color terms + wcs.amd_x_coeff[13]*mag + wcs.amd_x_coeff[14]*mag*mag
        ...
        eta = wcs.amd_y_coeff[0] * ymm + wcs.amd_y_coeff[1] * xmm + wcs.amd_y_coeff[2]
                + wcs.amd_y_coeff[3] * ymm2 + wcs.amd_y_coeff[4] * xmm * ymm + wcs.amd_y_coeff[5]
                * xmm2 + wcs.amd_y_coeff[6] * (x2y2) + wcs.amd_y_coeff[7] * ymm3
        ...
        /*
         * Ignore magnitude and color terms + wcs.amd_y_coeff[13]*mag + wcs.amd_y_coeff[14]*mag*mag
    ...
    public static Point2D.Double getPixels(double xpos, double ypos, WCSTransform wcs) {
        ...
        for (i = 0; i < MAX_ITERATIONS; i++) {
            ...
            x4 = x2 * x2;
            y4 = y2 * y2;
            f = wcs.amd_x_coeff[0] * xmm + wcs.amd_x_coeff[1] * ymm + wcs.amd_x_coeff[2]
                    + wcs.amd_x_coeff[3] * x2 + wcs.amd_x_coeff[4] * xy + wcs.amd_x_coeff[5] * y2
                    + wcs.amd_x_coeff[6] * x2y2 + wcs.amd_x_coeff[7] * x3 + wcs.amd_x_coeff[8]
                    * x2y + wcs.amd_x_coeff[9] * y2x + wcs.amd_x_coeff[10] * y3
                    + wcs.amd_x_coeff[11] * xmm * x2y2 + wcs.amd_x_coeff[12] * xmm * cjunk;
            /*
             * magnitude and color terms ignored + wcs.amd_x_coeff[13]*mag +
            ...
            fx = wcs.amd_x_coeff[0] + wcs.amd_x_coeff[3] * 2.0 * xmm + wcs.amd_x_coeff[4] * ymm
                    + wcs.amd_x_coeff[6] * 2.0 * xmm + wcs.amd_x_coeff[7] * 3.0 * x2
                    + wcs.amd_x_coeff[8] * 2.0 * xy + wcs.amd_x_coeff[9] * y2 + wcs.amd_x_coeff[11]
                    * (3.0 * x2 + y2) + wcs.amd_x_coeff[12] * (5.0 * x4 + 6.0 * x2 * y2 + y4);
            /*
             * magnitude and color terms ignored wcs.amd_x_coeff[16]*mag +
             * wcs.amd_x_coeff[17]*mag*2.0*xmm + wcs.amd_x_coeff[18]*mag*(3.0*x2+y2);
             */

            /* Derivative of X model wrt y */
            fy = wcs.amd_x_coeff[1] + wcs.amd_x_coeff[4] * xmm + wcs.amd_x_coeff[5] * 2.0 * ymm
                    + wcs.amd_x_coeff[6] * 2.0 * ymm + wcs.amd_x_coeff[8] * x2 + wcs.amd_x_coeff[9]
                    * 2.0 * xy + wcs.amd_x_coeff[10] * 3.0 * y2 + wcs.amd_x_coeff[11] * 2.0 * xy
                    + wcs.amd_x_coeff[12] * 4.0 * xy * x2y2;
            /*
             * magnitude and color terms ignored wcs.amd_x_coeff[17]*mag*2.0*ymm +
             * wcs.amd_x_coeff[18]*mag*2.0*xy;
             */

            /* Y plate model */
            g = wcs.amd_y_coeff[0] * ymm + wcs.amd_y_coeff[1] * xmm + wcs.amd_y_coeff[2]
                    + wcs.amd_y_coeff[3] * y2 + wcs.amd_y_coeff[4] * xy + wcs.amd_y_coeff[5] * x2
                    + wcs.amd_y_coeff[6] * x2y2 + wcs.amd_y_coeff[7] * y3 + wcs.amd_y_coeff[8]
                    * y2x + wcs.amd_y_coeff[9] * x2y + wcs.amd_y_coeff[10] * x3
                    + wcs.amd_y_coeff[11] * ymm * x2y2 + wcs.amd_y_coeff[12] * ymm * cjunk;
            /*
             * magnitude and color terms ignored wcs.amd_y_coeff[13]*mag +
             * wcs.amd_y_coeff[14]*mag*mag + wcs.amd_y_coeff[15]*mag*mag*mag +
             * wcs.amd_y_coeff[16]*mag*ymm + wcs.amd_y_coeff[17]*mag*x2y2 +
             * wcs.amd_y_coeff[18]*mag*ymm*x2y2 + wcs.amd_y_coeff[19]*color;
             */

            /* Derivative of Y model wrt x */
            gx = wcs.amd_y_coeff[1] + wcs.amd_y_coeff[4] * ymm + wcs.amd_y_coeff[5] * 2.0 * xmm
                    + wcs.amd_y_coeff[6] * 2.0 * xmm + wcs.amd_y_coeff[8] * y2 + wcs.amd_y_coeff[9]
                    * 2.0 * xy + wcs.amd_y_coeff[10] * 3.0 * x2 + wcs.amd_y_coeff[11] * 2.0 * xy
                    + wcs.amd_y_coeff[12] * 4.0 * xy * x2y2;
            /*
             * magnitude and color terms ignored wcs.amd_y_coeff[17]*mag*2.0*xmm +
             * wcs.amd_y_coeff[18]*mag*ymm*2.0*xmm;
             */

            /* Derivative of Y model wrt y */
            gy = wcs.amd_y_coeff[0] + wcs.amd_y_coeff[3] * 2.0 * ymm + wcs.amd_y_coeff[4] * xmm
                    + wcs.amd_y_coeff[6] * 2.0 * ymm + wcs.amd_y_coeff[7] * 3.0 * y2
                    + wcs.amd_y_coeff[8] * 2.0 * xy + wcs.amd_y_coeff[9] * x2 + wcs.amd_y_coeff[11]
                    * (x2 + 3.0 * y2) + wcs.amd_y_coeff[12] * (5.0 * y4 + 6.0 * x2 * y2 + x4);
            /*
             * magnitude and color terms ignored wcs.amd_y_coeff[16]*mag +
             * wcs.amd_y_coeff[17]*mag*2.0*ymm + wcs.amd_y_coeff[18]*mag*(x2+3.0*y2);
             */
    ...
    /*
     * Mar 6 1995 Original version of this code May 4 1995 Fix eta cross terms which were all in y
     * Jun 21 1995 Add inverse routine Oct 17 1995 Fix inverse routine (degrees -> radians) Nov 7
     * 1995 Add half pixel to image coordinates to get astrometric plate coordinates Feb 26 1996 Fix
     * plate to image pixel conversion error
     */
}
